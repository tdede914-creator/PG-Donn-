...
// Kunci SATU device fingerprint yang UTUH & konsisten.
// INVARIANT KRITIS: fingerprint yang dikirim saat login (request + verify OTP)
// HARUS identik byte-for-byte dengan yang dikirim saat fetch mutasi. Kalau ada
// satu field pun beda, OrderKuota meng-invalidate token dan membalas
// "Sesi telah berakhir" walau OTP baru saja sukses. phone_uuid WAJIB = prefix
// app_reg_id (persis seperti profil device di library referensi jywa-orkut).
function lockFingerprint(overrides = {}) {
  const regId = overrides.appRegId || OK_CONSTANTS.app_reg_id;
  const fp = getConstants({ ...overrides, appRegId: regId });
  fp.phone_uuid = String(regId).split(':')[0];
  return fp;
}

// Tambahkan SELURUH field fingerprint ke form login. Sebelumnya login cuma
// mengirim 5 field (tanpa phone_uuid/phone_model/phone_android_version/ui_mode)
// sehingga token ter-bind ke device yang beda dari saat fetch mutasi.
function appendFingerprint(form, fp) {
  form.append('app_reg_id', fp.app_reg_id);
  form.append('phone_uuid', fp.phone_uuid);
  form.append('phone_model', fp.phone_model);
  form.append('phone_android_version', fp.phone_android_version);
  form.append('app_version_code', fp.app_version_code);
  form.append('app_version_name', fp.app_version_name);
  form.append('ui_mode', fp.ui_mode);
}

// Fingerprint internal (snake_case) → bentuk camelCase untuk disimpan di
// credentials JSON, supaya fetch mutasi mereproduksi fingerprint yang sama.
function fingerprintToCreds(fp) {
  return {
    appRegId: fp.app_reg_id,
    phoneUuid: fp.phone_uuid,
    phoneModel: fp.phone_model,
    phoneAndroidVersion: fp.phone_android_version,
    appVersionCode: fp.app_version_code,
    appVersionName: fp.app_version_name,
  };
}
...
async function requestOtp({ username, password, appRegId }) {
  ...
  const fp = lockFingerprint({ appRegId: regId });
  const form = new URLSearchParams();
  form.append('username', username);
  form.append('password', password);
  appendFingerprint(form, fp);
  ...
  return {
    ...
    appRegId: regId,
    // Fingerprint lengkap yang dipakai saat login — WAJIB dipakai ulang persis
    // sama saat verify OTP & fetch mutasi.
    fingerprint: fingerprintToCreds(fp),
  };
}

// -- Step 2: verify OTP (dapat token) --------------------------------------
async function verifyOtp({ username, otp, appRegId, fingerprint }) {
  if (!otp) throw new Error('OTP wajib');
  const regId = appRegId || fingerprint?.appRegId || OK_CONSTANTS.app_reg_id;

  // Reproduksi fingerprint yang sama dengan step request OTP.
  const fp = lockFingerprint({ ...(fingerprint || {}), appRegId: regId });
  const form = new URLSearchParams();
  form.append('username', username);
  ...
  return {
    ...
    appRegId: regId,
    // Kembalikan fingerprint utuh supaya route bisa simpan ke credentials
    // PERSIS seperti yang dipakai login → fetch mutasi tidak akan ditolak.
    fingerprint: fingerprintToCreds(fp),
  };
}
...
async function fetchQrisHistory(creds, options = {}) {
  ...
  // lockFingerprint memaksa phone_uuid = prefix app_reg_id, jadi credentials
  // lama yang phone_uuid-nya tidak konsisten tetap menghasilkan fingerprint
  // yang valid (penyebab umum "Sesi telah berakhir").
  const constants = lockFingerprint(creds);
