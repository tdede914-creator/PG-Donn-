...
│   ├── providers/
│   │   ├── index.js               # Registry
│   │   ├── orderkuota_jywa.js     # Adapter OrderKuota (login OTP) — deprecated
│   │   └── dana_bisnis.js         # Adapter DANA Bisnis (token/cookie sesi)
...
> **Catatan penting:** OrderKuota QRIS **sudah tutup**, jadi provider utama
> sekarang adalah **`dana_bisnis`** (baca mutasi dari akun DANA Bisnis lewat
> token/cookie sesi). Adapter `orderkuota_jywa` masih di-bundle tapi deprecated.
> **Setup DANA Bisnis dijelaskan lengkap di [`DANA-SETUP.md`](./DANA-SETUP.md)**
> — termasuk cara mendapatkan token, mengisi credentials, dan mengkalibrasi
> endpoint pakai tombol **Poll Now (debug)**. Bagian di bawah ini hanya ringkasan.
...
- **Login**: pakai panel **Login DANA Bisnis** (OTP) di halaman Providers, atau
  import token manual hasil capture. Detail lengkap → **[`DANA-SETUP.md`](./DANA-SETUP.md)**.
- **Credentials (JSON)** — bentuk minimal:

```json
{
  "apiBase": "https://api.saas.dana.id",
  "txPath": "/v1/merchant/transactions",
  "accessToken": "TOKEN_ASLI_HASIL_CAPTURE",
  "merchantId": "OPSIONAL"
}
```

- Endpoint & bentuk response DANA **wajib diverifikasi** dengan nilai asli dari
  akunmu (bukan API publik resmi). Semua endpoint, header, parameter, dan
  pemetaan field (`fieldMap`) bisa diatur lewat credentials tanpa ubah kode.
- Alur kalibrasi: isi credentials → **Poll Now (debug)** → baca raw response →
  sesuaikan `txPath`/`fieldMap` → ulangi sampai mutasi terbaca. Lihat
  [`DANA-SETUP.md`](./DANA-SETUP.md) untuk referensi field lengkap &
  troubleshooting.
- Pencocokan tetap berdasar **nominal persis** (`totalAmount = amount + kode
  unik`), jadi pastikan `fieldMap.amount` menunjuk ke **gross amount** (yang
  dibayar pembeli).
